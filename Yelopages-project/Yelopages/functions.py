...
def getUserObject(username):
	...
	user = User(uid, userrecord[0][3], userrecord[0][1], userrecord[0][4], userrecord[0][5], userrecord[0][6], userrecord[0][8], profession, domain, apartment, locality, pincode)
	print("yes",user)
...
def completeProfile(username, phone, apartment, locality, pincode):
	...
	mycursor.execute(query, values)
	query = "UPDATE user SET phoneNumber = '{}', completeProfile = 1 WHERE username = '{}'".format(phone, username)
	mycursor.execute(query)
	mydb.commit()
